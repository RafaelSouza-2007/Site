...
function listar() {
  ...
  for (let pos in lista) {
    ...
    tabela.innerHTML += `<button id="botaoExcluir" style="top:${(Number(pos)*32.3) + 160}px;" class="fa fa-trash" onclick="excluir(${Number(pos)})"></button>`;
    tabela.innerHTML += `<button id="botaoEditar" style="top:${(Number(pos)*32.3) + 160}px;" class="fa fa-edit" onclick="editar(${Number(pos)})"></button>`;
  }
}
